...
这套框架不是纸上谈兵 —— 已经在第二种栈 (Rust + React) 上复用过, 框架本体一行没改, 证明八步法骨架和 `@rules` 追溯链不绑语言、不绑平台。

| 项目 | 领域 | 技术栈 | 状态 | 证明了什么 |
|------|------|--------|------|-----------|
| [`ai-frontend`](../../tree/ai-frontend) | Web 管理后台 | UmiJS + React + Antd + Vitest + Playwright | 参考实现 | 框架的原始诞生地 —— 闸门、`@rules`、追溯链等机制都是从这里长出来的 |
| [**Spider**](https://github.com/Caspian-Sun/spider) | 桌面端开发者工具 | Tauri 2 + Rust + React | 开发中 | **跨栈** + **元意义**: 这个产品**自身**就是把任意 `claude-code-workflow` 仓库可视化成看板 —— 用方法论造的、用来可视化方法论的工具。PRD 锚点从 JSDoc 平滑迁移到 Rust `///` doc, IPC 引入了新的"禁硬编码"红线 |

### 仍在解决的诚实缺口

- **Spider 16 commits 之后节奏放缓** —— 反映出八步法的前置 PRD 成本对"边设计边实现的探索性工具"偏高, 是一个值得写进"不适用场景"的真实边界案例
- **Token 经济性目前还没量化** —— 在推荐给"成本敏感团队"之前需要补这块数据
- **目前只验证过两种栈** —— `ai-backend` / `ai-data` 分支在路线图上, 用于扩大证据面

> 完整复盘报告: [Spider 方法论复盘](https://github.com/Caspian-Sun/spider/blob/main/docs/methodology-report.md)
